...
def get_user_ids(config, users):
    ...
    for user in users:
        try:
            ...
            if response.status_code != 200:
                logging.warn(
                    f"Got response other than 200 from user id search: {response.status_code}"
                )
            else:
...
for broadcaster_id in gotten_user_ids:
    for start_day, end_day in zip(start_dates, end_dates):
        url1 = "https://api.twitch.tv/helix/clips?broadcaster_id={}&started_at={}&ended_at={}".format(
            broadcaster_id, start_day, end_day
        )
        ...
        logging.info(
            f"Got reponse: {response.status_code} from request without cursor (first request)"
        )
        data = response.json()
        for value in data["data"]:
            ...
            cur.execute(
                "INSERT INTO clips (id, name, embed_url, broadcaster, creator_name, game_id, title, view_count, created_at, duration, added, available_twitch) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE view_count=VALUES(view_count), edited=VALUES(added)",
                (
                    table_id,
                    value["id"],
                    value["embed_url"],
        ...
        if "cursor" in data["pagination"]:
            ...
            while cursor:
                try:
                    url2 = "https://api.twitch.tv/helix/clips?broadcaster_id={}&started_at={}&ended_at={}&after={}".format(
                        broadcaster_id, start_day, end_day, cursor
                    )
                    ...
                    logging.info(
                        f"Got reponse: {response.status_code} from request with cursor"
                    )
...
for broadcaster_id in gotten_user_ids:
    url1 = (
        "https://api.twitch.tv/helix/videos?user_id={}&type=highlight&first=100".format(
            broadcaster_id
        )
    )
    logging.info(f"Opening url: {url1}")
    ...
    for value in data["data"]:
        ...
        parsed = today_is.strftime("%Y-%m-%d %H:%M:%S")
        cur.execute(
            "INSERT INTO highlights (id, title, url, user_name, description, thumbnail_url, viewable, view_count, created_at, duration, added, available_twitch) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE view_count=VALUES(view_count), edited=VALUES(added)",
            (
                high_id,
                value["title"],
                url1,
    ...
    if "cursor" in data["pagination"]:
        ...
        while cursor:
            try:
                url2 = "https://api.twitch.tv/helix/videos?user_id={}&type=highlight&first=100&after={}".format(
                    broadcaster_id, cursor
                )
                response1 = requests.get(
                    url2,
                    timeout=timeout_length,
